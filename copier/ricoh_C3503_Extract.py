...
class extract_copier_ricoh_c3503():
    ...
    def __init__(self) -> None:
        ...
        self.s = HTMLSession()
        loginForm = self.s.get(self.url_login,headers= self.headers)
        soup = BeautifulSoup(loginForm.text, 'html.parser')
        self.wimToken = soup.find("input",attrs={"name": "wimToken"})
        data = {
            'wimToken': '',
            'userid_work': '',
            'userid': 'YWRtaW4=',
        ...
        headers['Cookie'] = headers['Cookie'].format(self.s.cookies['risessionid'], "--")
        data['wimToken']= self.wimToken['value']
        self.r = self.s.post(self.url,data=data, headers =headers)
